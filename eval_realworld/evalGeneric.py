...
# Evitamos 'l1' porque liblinear no soporta multiclase (n_clases>=3).
LOGREG_PENALTY_GRID = ["l2"]
...
def load_dataset(csv_path=None, target_column=None, sep=","):
    ...
    # Carga inicial con separador indicado; si queda todo en 1 columna se intenta autodetección.
    sep_used = sep
    try:
        df = pd.read_csv(csv_path, sep=sep_used)
    except Exception:
        df = pd.read_csv(csv_path, sep=None, engine="python")
        sep_used = "auto"

    if df.shape[1] == 1 and sep_used != "auto":
        try:
            df_auto = pd.read_csv(csv_path, sep=None, engine="python")
    ...
    # Heurística simple para elegir la columna objetivo si no se pasó.
...
def predict_with_fallback(clf, q_x, builder_fn, params, s_x, s_y):
    """
    Ejecuta predict(); si hay PermissionError por hilos/procesos en Windows,
    desactiva paralelismo, reconstruye con n_jobs=1 y reentrena.
    """
    try:
        return clf.predict(q_x)
...
def add_classical_predictions(preds, prefix, s_x, s_y, q_x, seed, knn_params, svm_params, logreg_params, mlp_params, rf_params):
    ...
    preds[f"kNN{prefix}"] = predict_with_fallback(
        knn,
        q_x,
        builder_fn=lambda p: build_knn_classifier(
            n_neighbors=p["n_neighbors"],
    ...
    preds[f"RandomForest{prefix}"] = predict_with_fallback(
        rf,
        q_x,
        builder_fn=lambda p: build_rf_classifier(
            n_estimators=p["n_estimators"],
